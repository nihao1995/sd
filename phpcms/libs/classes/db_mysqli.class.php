...
final class db_mysqli {
    ...
    //************************************************************************************************************************************************************
    //2019/4/1添加
    public function spcSql($sql, $isreturn = 0, $returnArray=0)
    {
        $this->execute($sql);
        if($isreturn == 1)
        {
    ...
    //多表查询字段。
    //传入参数形式如：
    //moreTableSelect(array("zy_zymember"=>array("id", "userid"), "zy_zycoupon"=>array("*")), array("userid","shopid"), "userid=1")
    //2019/4/18补充大于等于3张表连接的时候on来left join,第一个个参数是主表和附表1参数连接，第二个参数是主表和附表2参数连接的桥梁，以此类推。
    //$where暂时只支持sql语句，以后再修改成数组类型，当B1,B2,B3都有同样参数的时候时，需要区别
    //2019/4/25$where 支持数组传递了
    //$returnArray == 1当只有一条数据的时候也返回数组形式，=0的时候只返回数据形式
    //$on不支持字典格式传入。因为键不能重名(改成了数组，而且支持分表联查)
    public function moreTableSelect($data, $on, $where="1", $limit = '', $order = '', $returnArray)
    {

		$s1 = "select sql_calc_found_rows ";
		$s2 = "";
		$order = $order == '' ? '' : ' ORDER BY '.$order;
		$limit = $limit == '' ? '' : ' LIMIT '.$limit;
		$num = 1;
		...
		foreach($data as $key=>$value)
		{
			...
			{
				$s1 .= $flag==0?" B".(string)$num.".".$row: ",B".(string)$num.".".$row;
				$flag++;
			}
//            $x = $num == 1?"":array_shift($on);
//            $s2 .= $num==1? "`".$key."` B".(string)$num." ": " left join `".$key."` B".(string)$num." on B1.".$x."=B".(string)$num.".".$x." ";
//            $num++;
			$k = $num == 1?"":array_shift($on);
			if(is_array($k))
            {
                if(is_array($k[0]))
                    $s2 .=  " left join `".$key."` B".(string)$num." on {$k[0][0]}.".$k[0][1]."=B".(string)$num.".".$k[1]." ";
                else
                    $s2 .=  " left join `".$key."` B".(string)$num." on B1.".$k[0]."=B".(string)$num.".".$k[1]." ";
            }
			else
			    $s2 .= $num==1? "`".$key."` B".(string)$num." ": " left join `".$key."` B".(string)$num." on B1.".$k."=B".(string)$num.".".$k." ";
			$num++;

		}
		$s1 .= " from ";
		$sql = $s1.$s2." where ".$where." ".$order." ".$limit.";";
		//return
		$info = $this->spcSql($sql, 1, $returnArray);
		$sql = "select found_rows()";
		$rowCount =  $this->spcSql($sql, 1, 0);
		return array($info, array_shift($rowCount));
		...
		if(is_string($data) && $data != '') {
			...
			foreach($data as $k=>$v) {
				switch (substr($v, 0, 2)) {
                    ...
                    case '*=':
                        $value = str_replace("*=", "", $v);
                        $fields[] = $this->add_special_char($k).'='.$value;
                        break;
                    case 'C='://字符串拼接
                        $value = str_replace("C=", "", $v);
                        $fields[] = $this->add_special_char($k).'= CONCAT('.$k.",'". $value.",') ";
                        break;
                    case 'DJ'://两个字段中的数据进行操作。
                        $value = str_replace("DJ", "", $v);
                        $fields[] = $this->add_special_char($k).'= '.$value." ";
                        break;
					default:
