...
class model {
    ...
    final public function select_or($where = '', $data = '*',  $limit = '', $order = '', $group = '',  $key='') {
	    if(is_array($where))
        {
            $sql = "1";
            ...
            return null;
        return $this->db->select($data, $this->table_name, $sql, $limit, $order, $group, $key);
    }
	//**************
    public function spcSql($sql, $isreturn = 0, $returnArray=0)
    {
        return $this->db->spcSql($sql, $isreturn, $returnArray);
    }
    public function moreTableSelect($data, $on, $where, $limit = '', $order = '',$returnArray = 0)
    {
        if (is_array($where)) $where = $this->sqls($where);
        return $this->db->moreTableSelect($data, $on, $where, $limit, $order,$returnArray);
    }
    //**************
	/**
	 * 查询多条数据并分页
    ...
    final public function delete_or($where = '') {
        if(is_array($where))
        {
            $sql = "1";
            foreach($where as $x=>$value)
            ...
            return null;
        return $this->db->delete($this->table_name, $sql);
    }
	/**
	 * 将数组转换为SQL语句
