...
spl_autoload_register(function($class_name) {
    $info = strpos($class_name,"classes");
    if(!($info || $info === "0"))
        return;
    $s = PC_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.str_replace('\\','/',$class_name).'.php';
    if(file_exists($s))
        require_once $s;
    else
        Error("没有找到".$class_name.'文件');
});
//api返回函数
...
function checkArg($neadData, $data)
{
        ...
        if(!empty($data[$key])|| $data[$key] == "0")
        {
			...
			switch($str)
            {
                ...
                case "2":
                    ...
                    break;
                case "3":
                    $info[$key] = $soleData[$key] = $data[$key];
                    break;
                case "4":
                    $info[$key] = "C=".$data[$key];
                    break;
    ...
    if(count($soleData)>1)
    {
        ...
        foreach ($soleData as $key=>$value)
            $message .= $key.' ';
        returnAjaxData('-1',$message.'只能传入其中一个');
    }
    return $info;
}
...
//2019/9/1日增加[true, 1, "错误返回内容"], 增加错误返回内容。
...
//            {
//                $info[$key] = $value[1] == 1? intval($_GET[ $key]): ($value[1] == 2? strtotime($_GET[$key]):($value[1] == 4?"C=".$_GET[$key]:$_GET[$key]));
//                if($value[1] == 3)
//                    $singleness[$key] = $_GET[$key];
//            }
//            else
//            {
//                if($value[0])
...
//            {
//                $info[$key] = $value[1] == 1? intval($_POST[$key]): ($value[1] == 2? strtotime($_POST[$key]):($value[1] == 4?"C=".$_POST[$key]:$_POST[$key]));
//                if($value[1] == 3)
//                    $singleness[$key] = $_GET[$key];
//            }
//            else
...
//        }
//    }
//    if(count($singleness)>1)
//    {
//        $message = '';
//        foreach ($singleness as $key=>$value)
//            $message .= $key.' ';
//        returnAjaxData('-1',$message.'只能传入其中一个');
//    }
//    return $info;
