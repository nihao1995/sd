    ...
    aj.post("index.php?m=zymanage&c=fileManage&a=getData&pc_hash=<?php echo $_GET["pc_hash"]?>",{page:1},function(data){
        window.parent.app12 = new Vue({
            ...
            data:{
                ...
                categoryoptions:data.data.category,
                category:'',
                VFTID:<?php echo $dataInfo["VFTID"]?>,
                titlename: '<?php echo $dataInfo["titlename"]?>',
                visible: false,
                FID:<?php echo $dataInfo["FID"]?>,
                uploadList: <?php echo $dataInfo["fileAddr"]?>,
                dataList:<?php echo $dataInfo["fileAddr"]?>,
                selected:''
            },
            methods:{
                ...
                upload:function()
                {
                    aj.post("index.php?m=zymanage&c=fileManage&a=editFileManage&pc_hash=<?php echo $_GET["pc_hash"]?>",{titlename:this.titlename, VFTID:this.VFTID, fileAddr:this.dataList, FID:this.FID},function(data){
                        if(data.code == 1)
                        {
            ...
            mounted:function()
            {
                for(index in this.categoryoptions)
                {
                    if(this.categoryoptions[index].VFTID == this.VFTID)
                        this.selected = this.categoryoptions[index].typename;
                }
            }
          })
     })
