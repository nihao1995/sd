...
<input type="hidden" value="user_manage" name="a">
...
<select name="certification">
    <option value="" selected="">已认证/未认证</option>
    <option value="1" <?php if ($_GET['certification']==1) {?>selected<?php }?>>已认证</option>
    <option value="2" <?php if ($_GET['certification']==2) {?>selected<?php }?>>未认证</option>
    <option value="3" <?php if ($_GET['certification']==3) {?>selected<?php }?>>待审核</option>
    <option value="4" <?php if ($_GET['certification']==4) {?>selected<?php }?>>认证失败</option>
</select>
...
<div class="explain-col search-form">
    <div style="margin-bottom: 5px;">
        <a href="?m=zymember&c=member&a=user_manage&time=1" class="btn" style="margin-right: 10px;">今日新增</a>
        <a href="?m=zymember&c=member&a=user_manage&time=2" class="btn">本月新增</a>
        <a href="?m=zymember&c=member&a=export_member" class="btn btn-warm" style="float: right;">导出数据</a>
    </div>
    用户数（人）：<?php echo $userNum?> <br>
    今日新增（人）：<?php echo $jr_userNum?> <br>
    本月新增（人）：<?php echo $by_userNum?> <br>
</div>
...
if(is_array($info)){
	...
	<tr <?php if($info['audit']==4){?> style="text-decoration: line-through; color: #999;"<?php } ?>>
        ...
        <td align="center">
            ...
            <?php echo $info['pid']?>
            <!-- <a href="javascript:void(0);" onclick="view('<?php echo $info['pid']?>')"><img src="<?php echo IMG_PATH?>admin_img/detail.png"></a> -->
        </td>
        ...
        <td align="center">
            <?php if($info['certification']==1){ ?>
                是
            <?php }?>
            <?php if($info['certification']==2){ ?>
                否
            <?php }?>
            <?php if($info['certification']==3){ ?>
                待审核
            <?php }?>
            <?php if($info['certification']==4){ ?>
                认证失败
            <?php }?>
        </td>
        <td align="center">
        ...
        <td align="center">
            ...
            <?php if($info['certification']==3){ ?>
                <a href="?m=zymember&c=member&a=certification_ok&userid=<?php echo $info['userid']?>&disable=1&types=<?php echo $info['enterprise']?>" onClick="return confirm('确认通过?')" class="btn btn-sm" style="padding-top: 0;padding-bottom: 0;">[通过]</a>

                <a href="?m=zymember&c=member&a=certification_ok&userid=<?php echo $info['userid']?>&disable=4&types=<?php echo $info['enterprise']?>" onClick="return confirm('确认不通过?')" class="btn btn-sm" style="padding-top: 0;padding-bottom: 0;">[不通过]</a>
            <?php }?>
...
function view(id) {
    window.top.art.dialog({
        id:'view',
        iframe:'?m=zymember&c=member&a=member_manage_view&type=1&userid='+id,
        title:'用户信息',
        width:'500',
        height:'500',
        lock:true
