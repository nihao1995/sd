...
<!--            <th>缩略图</th>-->
        ...
        <tbody>
            ...
            <template v-for="item in itemGet" v-model="itemGet">
             <tr>
                 <td><Checkbox :label="item.MSID"><span></span></Checkbox></td>
                 <td>{{item.MSID}}</td>
                 <td>{{item.userid}}</td>
                 <td>{{item.title}}</td>
<!--                 <td>{{item.content}}</td>-->
...
<!--                         <i-button type="info" @click="pass(item.MSID)" >通过</i-button>-->
<!--                         <i-button type="error" @click="reject(item.MSID)" >驳回</i-button>-->
<!--                     </template>-->
<!--                     <i-button type="info" @click="edit(item.MSID)" >编辑</i-button>-->
<!--                     <i-button type="error"  @click="del(item.MSID)">删除</i-button>-->
...
<script>
    ...
    aj.post("index.php?m=zysd&c=zysd&a=message_list&pc_hash=<?php echo $_GET["pc_hash"]?>",{page:'1'},function(data){
        if(data.code=='200')
        {
            app = new Vue({
                el: '#singleChoice',
                ...
                methods:{
                    ...
                    getData:function(page){
                        ...
                        aj.post("index.php?m=zysd&c=zysd&a=message_list&pc_hash=<?php echo $_GET["pc_hash"]?>",{page:page, userid:this.userid, owner_name:this.owner_name},function(data){
                            console.log(data);
                            that.page = page;
                    ...
                    pass:function(ID){
                        ...
                        layer.confirm('确定通过？', {icon: 1, title:'提示'}, function(index){
                            //do something
                            aj.post("index.php?m=zysd&c=zysd&a=fund_pass&pc_hash=<?php echo $_GET["pc_hash"]?>",{MSID:ID},function(data){
                                if(data.code == 200)
                                    that.getData(that.page);
                                else
                                    layer.msg(data.message);
                    ...
                    reject:function(ID){
                        ...
                        layer.confirm('确定驳回？', {icon: 2, title:'提示'}, function(index){
                            //do something
                            aj.post("index.php?m=zysd&c=zysd&a=fund_dismiss&pc_hash=<?php echo $_GET["pc_hash"]?>",{MSID:ID},function(data){
                                if(data.code == 200)
                                    that.getData(that.page);
                                else
                                    layer.msg(data.message);
