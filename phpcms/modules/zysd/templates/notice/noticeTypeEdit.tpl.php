        ...
        <div id="div_setting_2" class="contentList">
            <fieldset>
                ...
                <table  class="table_form">
                    <tbody>
                    <tr>
                        <th style="width: 120px">公告类型名称</th>
                        <td>
                            <i-input v-model="notice_type_name"  placeholder="请输入公告类型名称" type="text" style="width: 300px"></i-input>
                        </td>
                    </tr>
                    <tr>
                        <th style="width: 120px">排序</th>
                        <td>
                            <i-input v-model="sort"  placeholder="请输入序号" type="text" style="width: 300px"></i-input>
                        </td>
                    </tr>
                    <tr>
...
<script>
    ...
    window.parent.app12 = new Vue({
        el:"#app",
        data:{
            notice_type_name:"<?php echo $dataInfo['notice_type_name']?>",
            sort:"<?php echo $dataInfo['sort'] ?>",
            status:<?php if($dataInfo['status']){ echo 'true';}else{ echo 'false';} ?>
        },
        methods:{
            upload:function()
            {
                console.log(window.parent.app12.data)
                aj.post("index.php?m=zysd&c=zysd&a=edit_noticeType&pc_hash=<?php echo $_GET["pc_hash"]?>",{notice_type_name:this.notice_type_name, sort:this.sort,status:this.status,NTID:"<?php echo $_GET['ID'] ?>"},function(data){
                    if(data.code == 200)
                    {
