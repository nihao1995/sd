    ...
    <Checkbox-Group v-model="IDI">
        ...
        <tbody>
            <template v-for="item in itemGet" v-model="itemGet">
             <tr>
                 <td><Checkbox :label="item.RFID"><span></span></Checkbox></td>
                 <td>{{item.RFID}}</td>
                 ...
                 <td>{{item.addtime}}</td>
                 <td align="center">
                     <i-button type="info" @click="edit(item.RFID)" >编辑</i-button>
                     <i-button type="error"  @click="del(item.RFID)" style="margin-top: 3px;">删除</i-button>
                 </td>
             </tr>
            </template>
        </tbody>
            ...
            app = new Vue({
                ...
                methods:{
                    ...
                    getData:function(page){
                        var that = this;
                        console.log(122);
                        aj.post("index.php?m=zymanage&c=rules&a=getData&pc_hash=<?php echo $_GET["pc_hash"]?>",{ page:page, titlename:this.itemname},function(data){
//                            console.log(data.data.pageStart);
                    ...
                    del:function(FID){
                        ...
                        layer.confirm('确定删除？', {icon: 3, title:'提示'}, function(index){
                            //do something
                            aj.post("index.php?m=zymanage&c=rules&a=delData&pc_hash=<?php echo $_GET["pc_hash"]?>",{type:"zysinglechoice", RFID:FID},function(data){
                                if(data.code == 1)
                    ...
                    edit:function(FID){
                        ...
                        layer.open({
                            ...
                            shade: 0.8,
                            area: ['800px', '90%'],
                            content: 'index.php?m=zymanage&c=rules&a=editFileManage&pc_hash=<?php echo $_GET["pc_hash"]?>&RFID='+FID, //iframe的url
                            end: function () {
                                console.log(1);
                               that.getData(that.page);
                            }
                    ...
                    addFile:function(){
                        ...
                        layer.open({
                            ...
                            shade: 0.8,
                            area: ['800px', '90%'],
                            content: 'index.php?m=zymanage&c=rules&a=addFileManage&pc_hash=<?php echo $_GET["pc_hash"]?>', //iframe的url
                            end: function () {
                                console.log(1);
                                that.getData(that.page);
                            }
                    ...
                    delid:function()
                    {
                        ...
                        layer.confirm('确定删除？', {icon: 3, title:'提示'}, function(index){
                            //do something
                            aj.post("index.php?m=zymanage&c=rules&a=delID&pc_hash=<?php echo $_GET["pc_hash"]?>",{RFID:that.IDI},function(data){
                                if(data.code == 1) {
                                    that.getData(that.page);
                                    that.IDI = [];
                                    that.checkAll = false;
