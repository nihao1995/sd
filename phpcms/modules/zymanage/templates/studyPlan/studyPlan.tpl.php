    ...
    <Checkbox-Group v-model="IDI">
        ...
        <tbody>
            <template v-for="item in itemGet" v-model="itemGet">
             <tr>
                 <td><Checkbox :label="item.SPID"><span></span></Checkbox></td>
                 <td>{{item.SPID}}</td>
                 <td>{{item.titlename}}</td>
                 <td>{{item.typename}}</td>
                 <td >
                     <div class="layer-photos-demo" style="min-width: 108px">
                         <template v-if="item.FID != ''">
                             <template  v-for="(value, key) in item.FID">
                                 <p>{{value.titlename}}</p>
                             </template>
                         </template>
                     </div>
                 ...
                 <td >
                     <div class="layer-photos-demo" style="min-width: 108px">
                         <template v-if="item.VID != ''">
                             <template  v-for="(value, key) in item.VID">
                                 <p>{{value.titlename}}</p>
                             </template>
                         </template>
                     </div>
                 ...
                 <td>{{item.addtime}}</td>
                 <td align="center">
                     <i-button type="info" @click="edit(item.SPID)" >编辑</i-button>
                     <i-button type="error"  @click="del(item.SPID)" style="margin-top: 3px;">删除</i-button>
                 </td>
             </tr>
            </template>
        </tbody>
...
<script>
    ...
    aj.post("index.php?m=zymanage&c=studyPlan&a=getData&pc_hash=<?php echo $_GET["pc_hash"]?>",{ page:'1'},function(data){
        if(data.code=='1')
        {
            app = new Vue({
                el: '#singleChoice',
                ...
                methods:{
                    ...
                    getData:function(page){
                        var that = this;
                        aj.post("index.php?m=zymanage&c=studyPlan&a=getData&pc_hash=<?php echo $_GET["pc_hash"]?>",{ page:page, titlename:this.itemname, VFTID:this.VFTID},function(data){
//                            console.log(data.data.pageStart);
                    ...
                    del:function(SPID){
                        ...
                        layer.confirm('确定删除？', {icon: 3, title:'提示'}, function(index){
                            //do something
                            aj.post("index.php?m=zymanage&c=studyPlan&a=delData&pc_hash=<?php echo $_GET["pc_hash"]?>",{type:"zysinglechoice", SPID:SPID},function(data){
                                if(data.code == 1)
                    ...
                    edit:function(SPID){
                        ...
                        layer.open({
                            ...
                            shade: 0.8,
                            area: ['1300px', '99%'],
                            content: 'index.php?m=zymanage&c=studyPlan&a=editSPManage&pc_hash=<?php echo $_GET["pc_hash"]?>&SPID='+SPID, //iframe的url
                            end: function () {
                                console.log(1);
                               that.getData(that.page);
                            }
                    ...
                    getVFTID:function(value)
                    {
                        this.VFTID = value===null?'':value.VFTID;
                        console.log(this.VFTID);
                    },
                    addFile:function(){
                        var that = this;
                        layer.open({
                            ...
                            shade: 0.8,
                            area: ['1300px', '99%'],
                            content: 'index.php?m=zymanage&c=studyPlan&a=addStudyPlan&pc_hash=<?php echo $_GET["pc_hash"]?>', //iframe的url
                            end: function () {
                                console.log(1);
                                that.getData(that.page);
                            }
                    ...
                    delid:function()
                    {
                        ...
                        layer.confirm('确定删除？', {icon: 3, title:'提示'}, function(index){
                            //do something
                            aj.post("index.php?m=zymanage&c=studyPlan&a=delID&pc_hash=<?php echo $_GET["pc_hash"]?>",{SPID:that.IDI},function(data){
                                if(data.code == 1) {
                                    that.getData(that.page);
                                    that.IDI = [];
                                    that.checkAll = false;
