    ...
    <table width="100%" class="layui-table">
        ...
        <template v-for="item in itemGet" v-model="itemGet">
            <tr>
                ...
                <td align="center">
                    <a class="btn btn-info btn-sm" @click="edit(item.VTID)">编辑</a>
                    <a class="btn btn-danger btn-sm" @click="del(item.VTID)">删除</a>
                </td>
            </tr>
        </template>
        </tbody>
...
<script>
    function edit(id){
        window.top.art.dialog({
            id:'edit',
            iframe:"?m=zymanagement&c=shop&a=edit&id="+id,
            title:'用户信息',
            width:'800',
            height:'500',
            lock:true
    ...
    var layer = layui.layer,
        ...
        upload = layui.upload,
        table = layui.table;
    aj.post("index.php?m=zymanage&c=videoManage&a=getTypeData&pc_hash=<?php echo $_GET["pc_hash"]?>",{},function(data){
        if(data.code=='1')
        {
            console.log(data);
            app = new Vue({
                ...
                methods:{
                    ...
                    del:function(VTID){
                        ...
                        layer.confirm('确定删除？', {icon: 3, title:'提示'}, function(index) {
                            aj.post("index.php?m=zymanage&c=videoManage&a=delType&pc_hash=<?php echo $_GET["pc_hash"]?>", {VTID: VTID}, function (data) {
                                that.refresh();
                            })
                        })
                    },
                    edit:function(VTID){
                        ...
                        layer.open({
                            ...
                            shade: 0.8,
                            area: ['500px', '50%'],
                            content: 'index.php?m=zymanage&c=videoManage&a=editvideoManageType&pc_hash=<?php echo $_GET["pc_hash"]?>&VTID='+VTID, //iframe的url
                            end: function () {
                                console.log(1);
                                that.refresh();
                            }
                    ...
                    addType:function(){
                        ...
                        layer.open({
                            ...
                            shade: 0.8,
                            area: ['500px', '50%'],
                            content: 'index.php?m=zymanage&c=videoManage&a=addvideoManageType&pc_hash=<?php echo $_GET["pc_hash"]?>', //iframe的url
                            end: function () {
                                console.log(1);
                                that.refresh();
                            }
