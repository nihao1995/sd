...
<div  class="table-list" id="questionstype">
    ...
    <table width="100%" class="layui-table">
        ...
        <template v-for="item in itemGet" v-model="itemGet">
            <tr>
                <td align="center">{{item.VFTID}}</td>
                <td align="center">{{item.typename}}</td>
                <td align="center">
                    <template v-if="item.typeshow==1">
                ...
                <td align="center">
                    <div class="layer-photos-demo" >
                        <template v-if="item.thumb != ''">
                            <img :layer-src="item.thumb" :src="item.thumb" style="height:50px;width:50px;">
                        </template>
                    </div>
                </td>
                <td align="center">
                    <a class="btn btn-info btn-sm" @click="edit(item.VFTID)">编辑</a>
                    <a class="btn btn-danger btn-sm" @click="del(item.VFTID)">删除</a>
                </td>
            </tr>
        </template>
        </tbody>
...
<script>
    var layer = layui.layer,
        ...
        upload = layui.upload,
        table = layui.table;
    aj.post("index.php?m=zymanage&c=studyPlan&a=getTypeData&pc_hash=<?php echo $_GET["pc_hash"]?>",{},function(data){
        if(data.code=='1')
        {
            console.log(data);
            app = new Vue({
                ...
                methods:{
                    refresh:function(){
                        var bbb = this;
                        aj.post("index.php?m=zymanage&c=studyPlan&a=getTypeData&pc_hash=<?php echo $_GET["pc_hash"]?>",{},function(data) {
                            bbb.itemGet = data.data;
                            bbb.$nextTick(function(){
                                bbb.photo();
                            })
                            //
                    ...
                    del:function(VFTID){
                        ...
                        layer.confirm('确定删除？', {icon: 3, title:'提示'}, function(index) {
                            aj.post("index.php?m=zymanage&c=studyPlan&a=delType&pc_hash=<?php echo $_GET["pc_hash"]?>", {VFTID: VFTID}, function (data) {
                                that.refresh(that);
                            })
                        })
                    },
                    edit:function(VFTID){
                        ...
                        layer.open({
                            ...
                            shade: 0.8,
                            area: ['500px', '60%'],
                            content: 'index.php?m=zymanage&c=studyPlan&a=editManageType&pc_hash=<?php echo $_GET["pc_hash"]?>&VFTID='+VFTID, //iframe的url
                            end: function () {
                                console.log(1);
                                that.refresh();
                    ...
                    addType:function(){
                        ...
                        layer.open({
                            ...
                            shade: 0.8,
                            area: ['500px', '60%'],
                            content: 'index.php?m=zymanage&c=studyPlan&a=addManageType&pc_hash=<?php echo $_GET["pc_hash"]?>', //iframe的url
                            end: function () {
                                console.log(1);
                                that.refresh();
