...
<div id="singleChoice">
    <div class="subnav">
        <button class="layui-btn layui-btn-sm layui-btn-normal" @click="addVideo()">添加</button>
    </div>
    <div class='selectVue'>
    ...
    <Checkbox-Group v-model="IDI">
        ...
        <tbody>
            <template v-for="item in itemGet" v-model="itemGet">
             <tr>
                 <td><Checkbox :label="item.VID"><span></span></Checkbox></td>
                 <td>{{item.VID}}</td>
                 ...
                 <td >
                     ...
                     <template v-if="item.photo != ''">
                             <img :layer-src="item.photo" :src="item.photo" style="height:50px;width:50px;">
                     </template>
                     </div>
                 </td>
                 <td>
                     <i-button  @click="showvideo(item.videoAddr)" type="info">查看视频</i-button>
                 </td>
                 <td>{{item.addtime}}</td>
                 <td>{{item.typename}}</td>
                 <td align="center">
                     <i-button type="info" @click="edit(item.VID)" >编辑</i-button>
                     <i-button type="error"  @click="del(item.VID)" style="margin-top: 3px;">删除</i-button>
                 </td>
             </tr>
            </template>
        </tbody>
...
<script>
    ...
    aj.post("index.php?m=zymanage&c=videoManage&a=getData&pc_hash=<?php echo $_GET["pc_hash"]?>",{ page:'1'},function(data){
        if(data.code=='1')
        {
            app = new Vue({
                el: '#singleChoice',
                ...
                methods:{
                    ...
                    getData:function(page){
                        var that = this;
                        aj.post("index.php?m=zymanage&c=videoManage&a=getData&pc_hash=<?php echo $_GET["pc_hash"]?>",{ page:page, titlename:this.itemname, VFTID:this.VFTID},function(data){
//                            console.log(data.data.pageStart);
                    ...
                    del:function(VID){
                        ...
                        layer.confirm('确定删除？', {icon: 3, title:'提示'}, function(index){
                            //do something
                            aj.post("index.php?m=zymanage&c=videoManage&a=delData&pc_hash=<?php echo $_GET["pc_hash"]?>",{type:"zysinglechoice", VID:VID},function(data){
                                if(data.code == 1)
                    ...
                    edit:function(VID){
                        ...
                        layer.open({
                            ...
                            shade: 0.8,
                            area: ['800px', '90%'],
                            content: 'index.php?m=zymanage&c=videoManage&a=editVideoManage&pc_hash=<?php echo $_GET["pc_hash"]?>&VID='+VID, //iframe的url
                            end: function () {
                                console.log(1);
                               that.getData(that.page);
                            }
                    ...
                    addVideo:function(){
                        ...
                        layer.open({
                            ...
                            shade: 0.8,
                            area: ['800px', '90%'],
                            content: 'index.php?m=zymanage&c=videoManage&a=addVideoManage&pc_hash=<?php echo $_GET["pc_hash"]?>', //iframe的url
                            end: function () {
                                console.log(1);
                                that.getData(that.page);
                            }
                    ...
                    delid:function()
                    {
                        ...
                        layer.confirm('确定删除？', {icon: 3, title:'提示'}, function(index){
                            //do something
                            aj.post("index.php?m=zymanage&c=videoManage&a=delID&pc_hash=<?php echo $_GET["pc_hash"]?>",{VID:that.IDI},function(data){
                                if(data.code == 1) {
                                    that.getData(that.page);
                                    that.IDI = [];
                                    that.checkAll = false;
