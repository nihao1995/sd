    ...
    <Checkbox-Group v-model="IDI">
        ...
        <tbody>
            <template v-for="item in itemGet" v-model="itemGet">
             <tr>
                 <td><Checkbox :label="item.RID"><span></span></Checkbox></td>
                 <td>{{item.RID}}</td>
                 <td>{{item.titlename}}</td>
                 <td>{{item.abstract}}</td>
                 <td>{{item.addtime}}</td>
                 <td align="center">
                     <i-button type="info" @click="edit(item.RID)" >编辑</i-button>
                     <i-button type="error"  @click="del(item.RID)" style="margin-top: 3px;">删除</i-button>
                 </td>
             </tr>
            </template>
        </tbody>
...
<script>
    ...
    aj.post("index.php?m=zymanage&c=rules&a=getRulesData&pc_hash=<?php echo $_GET["pc_hash"]?>",{ page:'1'},function(data){
        if(data.code=='1')
        {
            app = new Vue({
                el: '#singleChoice',
                ...
                methods:{
                    ...
                    getData:function(page){
                        var that = this;
                        console.log(122);
                        aj.post("index.php?m=zymanage&c=rules&a=getRulesData&pc_hash=<?php echo $_GET["pc_hash"]?>",{ page:page, titlename:this.itemname},function(data){
//                            console.log(data.data.pageStart);
                    ...
                    del:function(FID){
                        ...
                        layer.confirm('确定删除？', {icon: 3, title:'提示'}, function(index){
                            //do something
                            aj.post("index.php?m=zymanage&c=rules&a=delRulesData&pc_hash=<?php echo $_GET["pc_hash"]?>",{ RID:FID},function(data){
                                if(data.code == 1)
                    ...
                    edit:function(FID){
                        ...
                        layer.open({
                            ...
                            shade: 0.8,
                            area: ['70%', '100%'],
                            content: 'index.php?m=zymanage&c=rules&a=editRulesManage&pc_hash=<?php echo $_GET["pc_hash"]?>&RID='+FID, //iframe的url
                            end: function () {
                                console.log(1);
                               that.getData(that.page);
                            }
                    ...
                    addFile:function(){
                        ...
                        layer.open({
                            ...
                            maxmin: false,
                            area: ['70%', '100%'],
                            content: 'index.php?m=zymanage&c=rules&a=addRulesManage&pc_hash=<?php echo $_GET["pc_hash"]?>', //iframe的url
                            end: function () {
                                console.log(1);
                                that.getData(that.page);
                            }
                    ...
                    delid:function()
                    {
                        ...
                        layer.confirm('确定删除？', {icon: 3, title:'提示'}, function(index){
                            //do something
                            aj.post("index.php?m=zymanage&c=rules&a=delIDI&pc_hash=<?php echo $_GET["pc_hash"]?>",{RID:that.IDI},function(data){
                                if(data.code == 1) {
                                    that.getData(that.page);
                                    that.IDI = [];
                                    that.checkAll = false;
